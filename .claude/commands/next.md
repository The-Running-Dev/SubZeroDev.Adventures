...
`AGENTS.md` § _Session boundaries_ is the authority on which is which, and this command does not
...
- **Deep-reasoning tier** — do not run it under this command's `sonnet`/`medium` routing even
  ...
  `AGENTS.md` § _Model, effort, and review budget_ is what gates that, and it gates this command
...
check of what is _outstanding_, which orientation alone does not answer:
...
| A branch is checked out with unpushed commits and no pull request                                              | Commit by named path, push, open the pull request. `AGENTS.md` § _Git and delivery_ delegates all four    |
...
| `design/30-slices.md` § _Outstanding_ is empty and every issue is closed                                       | Say the slice set is exhausted, and name `/reconcile` (**boundary**, `opus`/`high`) as what follows       |
